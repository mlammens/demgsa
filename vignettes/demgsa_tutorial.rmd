...
Windows formated batch script (`*.BAT`)
and a \*nix formated batch script (`*.sh`). 
These two files help the user to batch run the 
new *.mp files.

### Using the Windows formated batch script

The Windows formated batch script uses a "helper" batch script names 
...
First, let's varify that (at least some of) 
...
to results extraction. Other potential reasons for modificaiton include:
